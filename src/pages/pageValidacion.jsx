...
const PageValidacion = () => {
  ...
  const handleSendVerificationEmail = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3001/verification/send-email', {
        email: User.email,
      });
  ...
  const handleVerifyCode = async (token) => {
    try {
      const response = await axios.post(`http://localhost:3001/verification/verify-email?token=${token}`);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography variant="h6" color="white">
          {User?.email} {/* Asegúrate de que "User" tenga la propiedad "email" */}
        </Typography>
        ...
        {message && (
          <Typography variant="body2" color={message.includes('éxito') || message.includes('verificado') ? 'green' : 'red'} sx={{ marginBottom: 2 }}>
            {message}
          </Typography>
        )}
        ...
        {verified && (
          <Button
            ...
            onClick={() => navigate('/Inicio')} // Redirige al inicio usando navigate
          >
            Ir a Inicio
          </Button>
        )}
